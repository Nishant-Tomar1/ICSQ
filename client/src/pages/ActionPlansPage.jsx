...
function AllExpectationsTable({
  ...
  if (!data || !Array.isArray(data)) {
    return (
      <div className="p-4 text-slate-400 text-center">No data available.</div>
    );
  }
    ...
    <div className="space-y-6">
      {groupOrder.map((group) => {
        ...
        return (
          ...
          >
            <div
              className={`px-4 py-3 bg-gradient-to-r ${group.color} text-white font-semibold sticky top-0 z-10`}
            >
                ...
                <tbody className="divide-y divide-white/5">
                  {items.map((e, idx) => (
                    ...
                    >
                      ...
                      <td
                        className="px-4 py-3 text-sm text-slate-300 max-w-md truncate"
                        title={e.expectation}
                      >
                        {e.expectation?.length > 20
                          ? e.expectation.substring(0, 20) + "..."
                          : e.expectation}
                      </td>
      ...
      {!groups.detractor.length &&
        !groups.passive.length &&
        !groups.promoter.length && (
          <div className="p-6 text-slate-400 text-center bg-white/5 rounded-lg border border-white/10">
            No expectations found in{" "}
            {currentDepartment?.name || "your current department"}.
          </div>
        )}
    </div>
  );
...
function DepartmentExpectationsTable({
  data,
  departmentId,
  allDepartments,
  onAssign,
  ...
  data.forEach((category) => {
    ...
    category.departments.forEach((dept) => {
      ...
      if (
        !selectedDept ||
        dept.name.toLowerCase() !== selectedDept.name.toLowerCase()
      ) {
        return;
      }
                ...
                <tbody className="divide-y divide-white/5">
                  {items.map((e, idx) => (
                    ...
                    >
                      ...
                      <td
                        className="px-4 py-3 text-sm text-slate-300 max-w-md truncate"
                        title={String(e.expectation || "")}
                      >
                        {String(e.expectation || "").length > 20
                          ? String(e.expectation || "").substring(0, 20) + "..."
...
function ActionPlansPage() {
  ...
  const [isLoadingExpectationData, setIsLoadingExpectationData] =
    useState(false);
  ...
  const [detailedPlanPreviewModal, setDetailedPlanPreviewModal] =
    useState(false);
  ...
  const [selectedPlanForExpandedView, setSelectedPlanForExpandedView] =
    useState(null);
    ...
    try {
      ...
      if (currentUser.role === "hod") {
        const expresponse = await axios.get(
          `${Server}/analytics/expectation-data/${getCurrentDepartment()?._id}`,
          { withCredentials: true }
        );
  ...
  const memoizedAdminFilters = useMemo(
    () => adminFilters,
    [
      adminFilters.departmentId,
      adminFilters.categoryId,
      adminFilters.assignedTo,
      adminFilters.status,
    ]
  );
  ...
  const filteredPlans = useMemo(() => {
    ...
    if (categoryFilter !== "all") {
      filtered = filtered.filter(
        (plan) =>
          plan.category?.name === categoryFilter ||
          plan.category?.[0]?.name === categoryFilter
      );
    }
    if (searchQuery) {
  ...
  const handleDeleteActionPlan = async (plan) => {
    if (
      ...
      try {
        await axios.delete(`${Server}/action-plans/${plan._id}`, {
          withCredentials: true,
        });
  ...
  const handleCreateActionPlan = async (e) => {
    ...
    if (!createForm.expectations.trim()) {
      toast({
        title: "Error",
        description: "Expectations are required.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (
      !Array.isArray(createForm.assignedTo) ||
      !createForm.assignedTo.length
    ) {
      toast({
        title: "Error",
    ...
    try {
      ...
      const existingCategory = allCategories.find(
        (c) => c.name.toLowerCase() === selectedCategoryForForm.toLowerCase()
      );

      // Handle multiple user assignment
      const assignedUsers = Array.isArray(createForm.assignedTo)
      ...
      if (assignedUsers.length > 1) {
        ...
        await axios.post(
          `${Server}/action-plans/batch`,
          {
            ...
            originalSurveyRespondents:
              createForm.originalSurveyRespondents || [],
          },
          { withCredentials: true }
        );
      } else {
      ...
      toast({
        title: "Success",
        description: `Action plan assigned to ${userCount} ${userText}!`,
      });
    } catch (err) {
      ...
      toast({
        title: "Error",
        description:
          err.response?.data?.message || "Failed to create action plan.",
        variant: "destructive",
      });
    } finally {
      setIsSubmitting(false);
  ...
  const handleDetailedPlanAssign = async () => {
    ...
    try {
      const actionPlanData = {
        ...
        instructions:
          detailedPlanAssignForm.instructions || selectedDetailedPlan.steps,
      ...
      await axios.post(`${Server}/action-plans`, actionPlanData, {
        withCredentials: true,
      });
      ...
      toast({
        title: "Success",
        description: "Action plan assigned successfully!",
      });
    } catch (error) {
      console.error("Error assigning action plan:", error);
      toast({
        title: "Error",
        description: "Failed to assign action plan",
        variant: "destructive",
      });
    } finally {
      setIsSubmitting(false);
  ...
  if (currentUser.role === "user" || currentUser.role === "admin") {
    ...
    const handleActionSave = async (e) => {
      ...
      try {
        await axios.put(
          `${Server}/action-plans/${actionModalPlan._id}`,
          { actions: actionInput },
          { withCredentials: true }
        );
        ...
        toast({
          title: "Error",
          description: "Failed to update actions.",
          variant: "destructive",
        });
      } finally {
        setIsSubmitting(false);
      ...
      <div className="bg-[#29252c]">
          ...
          {filteredPlans.length === 0 && (
            <div className="flex flex-col items-center gap-4 mt-10">
              ...
              <div className="text-slate-200 text-xl font-medium">
                No action plans assigned to you
              </div>
                          ...
                          <TableRow
                            ...
                            className={`${
                              index % 2 === 0
                                ? "bg-gray-800/40"
                                : "bg-gray-900/40"
                            } hover:bg-blue-600/20 transition-all duration-200 border-b border-gray-700/30`}
                          >
                            <TableCell
                              ...
                              <div className="flex justify-center">
                                <Select
                                  ...
                                  onValueChange={async (value) => {
                                    ...
                                    try {
                                      await axios.patch(
                                        `${Server}/action-plans/${plan._id}/status`,
                                        { status: value },
                                        { withCredentials: true }
                                      );
                                      fetchData();
            ...
            {actionModalPlan && (
              <form onSubmit={handleActionSave} className="space-y-4">
                <div>
                  ...
                  <div className="font-semibold text-[#FFF8E7]">
                    {capitalizeFirstLetter(actionModalPlan.department?.name)}
                  </div>
                </div>
                <div>
                  <div className="text-xs text-gray-400 mb-1">Category</div>
                  <div className="font-semibold text-[#FFF8E7]">
                    {capitalizeFirstLetter(actionModalPlan.category?.name)}
                  </div>
                </div>
                <div>
                  <div className="text-xs text-gray-400 mb-1">Expectations</div>
                  <div className="text-[#FFF8E7] max-h-24 overflow-y-auto">
                    {actionModalPlan.expectations}
                  </div>
                </div>
                <div>
        ...
        <Dialog open={updatingStatusId !== null} onOpenChange={() => {}}>
          <DialogContent className="sm:max-w-md bg-gradient-to-br from-slate-800 to-slate-900 border-slate-700">
            <DialogHeader>
              <DialogTitle className="text-center text-white font-semibold">
                Updating Status
              </DialogTitle>
            </DialogHeader>
            <div className="flex flex-col items-center justify-center py-8">
              ...
              <p className="text-slate-300 text-center text-sm">
                Please wait while we update the action plan status...
              </p>
            </div>
          </DialogContent>
        </Dialog>
  ...
  const handleGenerateAIActionPlans = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${Server}/analytics/summarize-expectations/ai`,
        {
          params: {
            departmentId: getCurrentDepartment()?._id,
    ...
    return [
      {
        id: "fallback_1",
        summary:
          aiSummary.substring(0, 200) + (aiSummary.length > 200 ? "..." : ""),
        category: "General",
        categoryId: "",
        priority: "Medium",
        originalData: "Based on survey responses",
    ...
    try {
      ...
      try {
        const res = await axios.get(
          `${Server}/users/by-department/${getCurrentDepartment()?._id}`,
          { withCredentials: true }
        );
        setDepartmentUsers(res.data || []);
      } catch (error) {
      ...
      toast({
        title: "Success",
        description:
          "AI summary loaded for assignment. You can now assign it to a team member.",
        variant: "success",
      });
    } catch (error) {
  ...
  if (currentUser.role === "hod") {
    return (
      <div className="min-h-screen bg-[#29252c]">
            ...
            <div className="bg-gray-300 rounded-lg max-w-2xl w-full p-6">
              <div className="text-center mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Welcome to Action Plans!
                </h2>
                <p className="text-gray-600">
                  Let's get you started with managing action plans effectively.
                </p>
              </div>
                ...
                {currentStep === 1 && (
                  <div className="p-4 bg-blue-50 rounded-lg">
                    ...
                    <p className="text-blue-800 text-sm">
                      Start here to see all your action plans and their current
                      status. You can filter, search, and manage existing plans.
                    </p>
                  </div>
                )}
                {currentStep === 2 && (
                  <div className="p-4 bg-green-50 rounded-lg">
                    ...
                    <p className="text-green-800 text-sm">
                      Select a category to view expectations and create action
                      plans based on survey responses.
                    </p>
                  </div>
                )}
                {currentStep === 3 && (
                  <div className="p-4 bg-purple-50 rounded-lg">
                    <h3 className="font-semibold text-purple-900 mb-2">
                      🤖 Step 3: AI Action Plans
                    </h3>
                    <p className="text-purple-800 text-sm">
                      Use AI to automatically analyze survey responses, group
                      similar feedback, and generate actionable plans.
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-slate-300 text-xs">
                    AI analyzes expectations and groups similar responses
        ...
        <div className="px-6 py-8 bg-gradient-to-r from-[#29252c]/80 to-[#29252c]/60 border-b border-gray-700/30">
          <div className="max-w-7xl mx-auto">
            ...
            <p className="text-slate-300 text-lg">
              Create, manage, and track action plans for your department
            </p>
                ...
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <p className="text-slate-300">
                      Here's an overview of your action plans and quick actions
                      you can take.
                    </p>
                  </div>
                  <Button
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="p-4 bg-white/5 rounded-lg">
                          ...
                          <p className="text-slate-300 text-sm">
                            Check your assigned action plans, update status, and
                        ...
                        <div className="p-4 bg-white/5 rounded-lg">
                          ...
                          <p className="text-slate-300 text-sm">
                            Go to "Expectations" tab to select a category and
                            create action plans based on survey responses.
                          </p>
                        </div>
                        <div className="p-4 bg-white/5 rounded-lg">
                          <div className="flex items-center gap-2 mb-2">
                          ...
                          <p className="text-slate-300 text-sm">
                            Use "AI Action Plans" tab to automatically generate
                            action plans by analyzing survey responses and
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                ...
                <Card className="bg-[#29252c]/70 backdrop-blur-sm border border-gray-700 text-center hover:shadow-lg transition-all duration-200">
                  <CardContent className="p-4">
                    ...
                    <div className="text-2xl font-bold text-white">
                      {
                        actionPlans.filter((p) => p.status === "in-progress")
                          .length
                      }
                    </div>
                ...
                <CardHeader className="border-b border-gray-700/50 bg-gradient-to-r from-gray-800/50 to-gray-900/50">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-sm text-slate-300 bg-amber-500/20 px-3 py-1 rounded-full border border-amber-400/30">
                      {
                        actionPlans.filter(
                          (plan) => plan.assignedBy?._id === currentUser._id
                        ).length
                      }{" "}
                      Plans
                    </div>
                      ...
                      <div className="overflow-x-auto max-h-96 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-gray-800">
                        <table className="w-full min-w-[1400px] border-separate border-spacing-0">
                          <thead className="bg-gradient-to-r from-gray-800 to-gray-900 sticky top-0 z-20">
                            <tr>
                              <th className="px-4 py-4 text-center font-bold text-white bg-gradient-to-b from-gray-800 to-gray-900 border-r border-gray-700">
                                Department
                              </th>
                              <th className="px-4 py-4 text-center font-bold text-white bg-gradient-to-b from-gray-800 to-gray-900 border-r border-gray-700">
                                Category
                              ...
                              <th className="px-4 py-4 text-center font-bold text-white bg-gradient-to-b from-gray-800 to-gray-900">
                                Actions
                              </th>
                            </tr>
                          </thead>
                          <tbody>
                            {actionPlans
                              .filter(
                                (plan) =>
                                  plan.assignedBy?._id === currentUser._id
                              )
                              .sort((a, b) => {
                                const statusOrder = {
                                  pending: 0,
                    ...
                    <div className="text-center py-12">
                      <div className="flex flex-col items-center gap-4">
                        ...
                        <div className="text-slate-300 text-lg font-medium">
                          No action plans assigned by you
                        </div>
                        <div className="text-slate-400 text-sm">
                          Create action plans in the "Expectations" tab
                        </div>
                      </div>
                    </div>
                  )}
        ...
        {/* Main Content Area */}
        {activeTab !== "overview" && activeTab !== "ai-action-plans" && (
            ...
            <div className="max-w-7xl mx-auto">
              <div className="flex flex-col lg:flex-row gap-8">
                  ...
                  <Card className="bg-[#29252c]/70 backdrop-blur-sm border border-gray-700 shadow-xl mb-4">
                    ...
                    <CardContent className="p-4">
                      <div className="space-y-3">
                        <Button
                          variant={
                            viewMode === "categories" ? "primary" : "outline"
                          }
                        ...
                        <Button
                          variant={
                            viewMode === "departments" ? "primary" : "outline"
                          }
                          className={`w-full justify-start transition-all duration-200 ${
                        ...
                        <ul className="space-y-3">
                          {allCategories
                            .filter(
                              (cat) =>
                                !cat.department ||
                                String(cat.department) ===
                                  String(getCurrentDepartment()?._id)
                            )
                            .map((category) => (
                              <li key={category._id}>
                                <Button
                ...
                <section className="flex-1 flex flex-col gap-6">
                          ...
                          <CardHeader className="border-b border-white/10">
                            <div className="flex items-center justify-between">
                              <CardTitle className="text-xl font-semibold text-white flex items-center gap-3">
                                ...
                                <Badge
                                  variant="secondary"
                                  className="ml-2 bg-green-500/20 text-green-300 border-green-400/30"
                                >
                            ...
                            <p className="text-slate-300 text-sm mt-2">
                              Review, edit, and assign these AI-generated action
                              plans.
                            </p>
                          </CardHeader>
                      ...
                      <CardHeader className="flex gap-2 justify-between border-b border-gray-700">
                        ...
                        <div className="flex gap-2">
                          ...
                          <Button
                            ...
                            className={
                              ratingFilter === "detractor"
                                ? "bg-gradient-to-r from-red-600 to-orange-600 border-0"
                                : "bg-white/5 border-white/20 text-slate-200 hover:bg-white/10"
                            }
                          >
                            Detractor
                          ...
                          <Button
                            size="sm"
                            variant={
                              ratingFilter === "passive" ? "primary" : "outline"
                            }
                            onClick={() => setRatingFilter("passive")}
                            className={
                              ratingFilter === "passive"
                                ? "bg-gradient-to-r from-yellow-600 to-orange-600 border-0"
                                : "bg-white/5 border-white/20 text-slate-200 hover:bg-white/10"
                            }
                          >
                            Passive
                              ...
                              {expectationData.length === 0 && (
                                <div className="text-center py-8 text-slate-300">
                                  ...
                                  <p className="text-sm text-slate-400">
                                    No expectations data found for{" "}
                                    {getCurrentDepartment()?.name ||
                                      "your department"}
                                    . Please check if surveys have been
                                    completed or contact your administrator.
                                  </p>
                                </div>
                              )}
                            </>
                        ...
                        <CardTitle className="text-xl font-semibold text-white flex items-center justify-between">
                          ...
                          <div className="flex gap-2">
                            <Button
                              ...
                              className={
                                ratingFilter === ""
                                  ? "bg-gradient-to-r from-[goldenrod] to-amber-500 border-0"
                                  : "bg-white/5 border-white/20 text-slate-200 hover:bg-white/10"
                              }
                            >
                              All
                          ...
                          {isLoadingExpectationData ? (
                            ...
                            <HODExpectationsTable
                              ...
                              onAssign={(expObj) => {
                                setCreateModalOpen(true);
                                setSelectedCategoryForForm(
                                  expObj.category || selectedCategory
                                );
                                setCreateForm((f) => ({
                                  ...f,
                                  department: expObj.department || "",
                          ...
                          {isLoadingExpectationData ? (
                            <div className="flex items-center justify-center p-8">
                              <div className="flex items-center gap-3">
                                <div className="w-6 h-6 border-2 border-blue-500 border-t-transparent rounded-full animate-spin"></div>
                                <span className="text-slate-300">
                                  Loading department expectations...
                                </span>
                              </div>
                            </div>
                          ) : (
                      ...
                      >
                          ...
                          {allCategories.filter(
                            ...
                            <div>
                              ...
                              <Select
                                ...
                                onValueChange={(val) =>
                                  setSelectedCategoryForForm(val)
                                }
                        ...
                        <div>
                          ...
                          <div className="text-xs text-slate-400 mt-1">
                            Define the specific outcomes, goals, or deliverables
                            expected from this action plan
                          </div>
                        </div>
                        <div>
                          <label className="block text-sm font-medium mb-2 text-slate-200">
                          ...
                          />
                          <div className="text-xs text-slate-400 mt-1">
                            Provide step-by-step guidance, resources, or
                            additional context for completing the action plan
                          </div>
                        </div>
                        <div>
                          <label className="block text-sm font-medium mb-2 text-slate-200">
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="flex items-center gap-3">
                              <Select
                                value=""
                                onValueChange={(val) => {
                                  const currentAssignedTo = Array.isArray(
                                    createForm.assignedTo
                                  )
                                    ? createForm.assignedTo
                                    : [];
                                ...
                                options={departmentUsers
                                  .filter((user) => {
                                    const currentAssignedTo = Array.isArray(
                                      ...
                                      ? createForm.assignedTo
                                      : [];
                                    return !currentAssignedTo.includes(
                                      user._id
                                    );
                                  })
                                  .map((user) => ({
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  // Select all users
                                  const allUserIds = departmentUsers.map(
                                    (user) => user._id
                                  );
                                  setCreateForm({
                                    ...createForm,
                  ...
                  {/* Preview Modal */}
                  <Dialog
                    open={detailedPlanPreviewModal}
                    onOpenChange={setDetailedPlanPreviewModal}
                  >
                      ...
                      {selectedDetailedPlan && (
                        <div className="space-y-6">
                          ...
                          <div className="bg-white/5 p-5 rounded-lg border border-white/10">
                            ...
                            <div className="space-y-2">
                              {selectedDetailedPlan.expectation
                                .split("\n")
                  ...
                  {/* Edit Modal */}
                  <Dialog
                    open={detailedPlanEditModal}
                    onOpenChange={setDetailedPlanEditModal}
                  >
                  ...
                  {/* Assign Modal */}
                  <Dialog
                    open={detailedPlanAssignModal}
                    onOpenChange={setDetailedPlanAssignModal}
                  >
                    <DialogContent className="bg-[#232026]/90 max-w-4xl max-h-[90vh] overflow-y-auto">
                      <DialogHeader>
                        <DialogTitle className="text-white flex items-center gap-2">
                          ...
                          <div className="p-4 bg-white/5 rounded-lg border border-white/10">
                            <div className="flex items-center gap-2">
                              ...
                              <Select
                                ...
                                onValueChange={(val) =>
                                  handleDetailedPlanAssignChange(
                                    "categoryId",
                                    val
                                  )
                                }
                              ...
                              {!usersLoading &&
                                (!departmentUsers ||
                                  ...
                                  <div className="text-xs text-red-400 mt-1">
                                    No users available to assign in this
                                    department.
                                  </div>
                                )}
                            </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="p-4 bg-gray-800/50 rounded-lg border border-gray-600">
                      ...
                      <p className="text-white font-medium">
                        {capitalizeFirstLetter(
                          selectedPlanForExpandedView.department?.name
                        )}
                      </p>
                    </div>
                    <div className="p-4 bg-gray-800/50 rounded-lg border border-gray-600">
                      ...
                      <p className="text-white font-medium">
                        {capitalizeFirstLetter(
                          selectedPlanForExpandedView.category?.name
                        )}
                      </p>
                    </div>
                    <div className="p-4 bg-gray-800/50 rounded-lg border border-gray-600">
                      ...
                      <p className="text-white font-medium">
                        {selectedPlanForExpandedView.assignedTo?.name}
                      </p>
                    </div>
                    <div className="p-4 bg-gray-800/50 rounded-lg border border-gray-600">
                      <h3 className="text-sm font-semibold text-gray-400 mb-2">
                      ...
                      <p className="text-white font-medium">
                        {new Date(
                          selectedPlanForExpandedView.targetDate
                        ).toLocaleDateString("en-GB")}
                      </p>
                    </div>
                  </div>
                      ...
                      <Select
                        ...
                        onValueChange={async (value) => {
                          ...
                          try {
                            ...
                            fetchData();
                            setSelectedPlanForExpandedView((prev) => ({
                              ...prev,
                              status: value,
                            }));
                            toast({
                        ...
                        disabled={
                          updatingStatusId === selectedPlanForExpandedView._id
                        }
                      />
                    </div>
                  </div>
                  ...
                  <div className="p-4 bg-gray-800/50 rounded-lg border border-gray-600">
                    ...
                    <p className="text-white whitespace-pre-wrap">
                      {selectedPlanForExpandedView.expectations}
                    </p>
                  </div>
                  ...
                  <div className="p-4 bg-gray-800/50 rounded-lg border border-gray-600">
                    ...
                    <p className="text-white whitespace-pre-wrap">
                      {selectedPlanForExpandedView.actions ||
                  ...
                  <div className="p-4 bg-gray-800/50 rounded-lg border border-gray-600">
                    ...
                    <p className="text-white whitespace-pre-wrap">
                      {selectedPlanForExpandedView.instructions ||
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="p-4 bg-gray-800/50 rounded-lg border border-gray-600">
                      ...
                      <p className="text-white font-medium">
                        {selectedPlanForExpandedView.assignedBy?.name}
                      </p>
                      <p className="text-gray-400 text-sm">
                        {new Date(
                          selectedPlanForExpandedView.createdAt
                        ).toLocaleDateString("en-GB")}
                      </p>
                    </div>
                    <div className="p-4 bg-gray-800/50 rounded-lg border border-gray-600">
                      ...
                      <p className="text-white font-medium">
                        {new Date(
                          selectedPlanForExpandedView.updatedAt
                        ).toLocaleDateString("en-GB")}
                      </p>
                    </div>
                  </div>
        ...
        {activeTab === "ai-action-plans" && (
          <div className="px-6 py-6">
            <div className="max-w-7xl mx-auto">
              <div className="mb-8">
                <h2 className="text-2xl font-bold text-white mb-2">
                  🤖 AI Expectations Summarizer
                </h2>
                <p className="text-slate-300 text-sm mt-2">
                  AI will analyze all survey responses from your current
                  department to generate simple, point-wise expectations based
                  on priority focus.
                </p>
                <p className="text-slate-400 text-xs mt-1">
                  Expected format: One expectation per line (e.g., "Improve
                  communication", "Reduce response time", etc.)
                </p>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-slate-300 text-xs leading-relaxed">
                        While we strive for accuracies around +90%, please note
                        that automated sentiment analysis may not fully capture
                        nuances in language, tone, or context. The results
                        should be used as a guide rather than an absolute
                        metric, and we recommend reviewing individual responses
                    ...
                    <div>
                      <div className="space-y-3">
                        <div>
                          ...
                          >
                            <option value="all" className="bg-[#29252c]">
                              All Priorities
                            </option>
                            <option value="high" className="bg-[#29252c]">
                              High Priority (Low Ratings)
                            </option>
                            <option value="medium" className="bg-[#29252c]">
                              Medium Priority
                            </option>
                            <option value="low" className="bg-[#29252c]">
                              Low Priority (High Ratings)
                            </option>
                          </select>
                        </div>
                      </div>
                  ...
                  <Card className="bg-gradient-to-br from-[#29252c]/80 to-[#1f1a23]/90 backdrop-blur-sm border border-gray-700/60 shadow-2xl ">
                    <CardHeader className="border-b border-gray-700 bg-gradient-to-r from-gray-800/50 to-gray-900/50">
                      ...
                      <p className="text-sm text-gray-400 mt-3 leading-relaxed">
                        Simple, point-wise expectations generated by AI analysis
                        of survey responses. Click "Assign" to assign to users
                        or "Edit" to modify the summary.
                      </p>
                    </CardHeader>
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        {aiGeneratedPlans &&
                          aiGeneratedPlans.map((plan, index) => (
                            <div
                              key={index}
                              className="bg-gradient-to-br from-gray-800/80 to-gray-900/80 rounded-2xl border border-gray-600/50 shadow-xl hover:shadow-2xl transition-all duration-300 hover:border-blue-400/40 hover:scale-[1.02] backdrop-blur-sm overflow-hidden"
                            >
                                  ...
                                  <div className="flex flex-col gap-2">
                                    ...
                                    {plan.priority && (
                                      <div className="flex items-center gap-2">
                                        ...
                                        <span
                                          className={`text-white text-base font-semibold px-3 py-2 rounded-md border ${
                                            plan.priority === "High"
                              ...
                              <div className="p-6 space-y-6">
                                ...
                                {plan.sourceResponses &&
                                  plan.sourceResponses.length > 0 && (
                                    <div>
                                      <h4 className="text-white font-semibold mb-3 text-sm uppercase tracking-wide">
                                        SOURCE RESPONSES:
                                      </h4>
                                      <div className="space-y-2">
                                        {plan.sourceResponses.map(
                                          (response, responseIndex) => (
                                            <div
                                              key={responseIndex}
                                              className="flex items-start gap-3 p-3 bg-gray-700/20 rounded-lg border border-gray-600/20"
                                            >
                    ...
                    <CardContent className="p-8">
                      <div className="text-center py-12">
                        ...
                        <h3 className="text-2xl font-bold text-white mb-4">
                          All Expectations Already Assigned
                        </h3>
                        <p className="text-slate-300 text-lg leading-relaxed max-w-2xl mx-auto mb-6">
                          Great work! All expectations from the survey data
                          analysis already have action plans assigned to team
                          members. There are no new summaries to display at this
                          time.
                        </p>
                        <div className="bg-blue-600/10 border border-blue-400/20 rounded-lg p-4 max-w-xl mx-auto">
                          <p className="text-blue-300 text-sm">
                            💡 <strong>Tip:</strong> You can view all existing
                            action plans in the "Overview" tab or try a
                            different priority focus to see if there are
                  ...
                  <Card className="bg-gradient-to-br from-[#29252c]/80 to-[#1f1a23]/90 backdrop-blur-sm border border-gray-700/60 shadow-2xl">
                    <CardContent className="p-8">
                      <div className="text-center py-12">
                        ...
                        <p className="text-slate-300 text-lg leading-relaxed max-w-2xl mx-auto mb-6">
                          No expectations were found in the survey data for the
                          selected priority focus. This might be because there
                          are no survey responses yet or the priority filter is
                          too restrictive.
                        </p>
                        <div className="bg-yellow-600/10 border border-yellow-400/20 rounded-lg p-4 max-w-xl mx-auto">
                          <p className="text-yellow-300 text-sm">
                            💡 <strong>Tip:</strong> Try changing the priority
                            focus to "All Priorities" or check if there are
                            survey responses for this department.
                          </p>
                        </div>
                      </div>
                    </CardContent>
              ...
              <Dialog
                open={aiPlanEditModalOpen && editingPlan}
                onOpenChange={(open) => {
                  if (!open) {
                    setAiPlanEditModalOpen(false);
                    setEditingPlan(null);
                      ...
                      <div className="space-y-8">
                        ...
                        <div>
                          ...
                          <Select
                            ...
                            onValueChange={(val) =>
                              setEditingPlan({ ...editingPlan, priority: val })
                            }
                            options={[
                        ...
                        <div>
                          ...
                          <div className="space-y-3">
                            {editingPlan.recommendedActions &&
                              editingPlan.recommendedActions.map(
                                (action, index) => (
                                  <div
                                    key={index}
                                  ...
                                  >
                                    ...
                                    <Button
                                      ...
                                      onClick={() => {
                                        const newActions =
                                          editingPlan.recommendedActions.filter(
                                            (_, i) => i !== index
                                          );
                                        setEditingPlan({
                                          ...editingPlan,
                    ...
                    <div className="space-y-6">
                      ...
                      <div>
                        ...
                        <div className="px-3 py-2 bg-gray-700/50 border border-gray-600 rounded-md text-slate-200">
                          {allCategories.find(
                            (cat) => cat._id === selectedCategoryForForm
